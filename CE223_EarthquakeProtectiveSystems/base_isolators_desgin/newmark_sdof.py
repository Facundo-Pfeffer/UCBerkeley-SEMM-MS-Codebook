...
This is a self-contained copy of the CE225 HW11 utility, kept here so that
base-isolator response scripts can depend on a stable, well-documented
implementation without importing across course directories.

Governing equation (relative coordinates):

    m * u¨(t) + c * u˙(t) + k * u(t) = p(t)

For base-isolated systems under ground acceleration u¨_g(t), a common choice is
to work in relative displacement u(t) and take the effective forcing as
    p(t) = -m * u¨_g(t),
so that the same routine can be used with time histories of ground motion.
"""
...
def newmark_sdof(
    ...
    p : np.ndarray
        Vector of forcing function (1D, same length as time).
    dt : float
        Scalar time step.
    u0 : float
    ...
    if method_lower == "constant":
        ...
        beta = 1.0 / 6.0  # Linear acceleration
        gamma = 1.0 / 2.0
    else:
        raise ValueError(
