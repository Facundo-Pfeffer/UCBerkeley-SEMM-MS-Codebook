...
def save_figure_with_template(fig, filename, title, div_id='plotly-plot', include_plotlyjs='cdn', explanation=None):
    """
    Save figure to HTML wrapped in website template with navigation.
    
    This function generates a Plotly HTML file, extracts the plot div and scripts,
    and wraps them in the website template with navigation.
    
    Args:
        fig: Plotly figure object
        filename: Output filename
        title: Page title (displayed in browser tab and as h1 on page)
        div_id: ID for the plot div (default: 'plotly-plot')
        include_plotlyjs: How to include plotly.js ('cdn', True, False)
        explanation: Optional HTML string with explanation text to display below the plot
    """
    import tempfile
    import os
    import re
    
    # Generate plot HTML to temporary file
    with tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False, encoding='utf-8') as tmp:
        tmp_path = tmp.name
    
    try:
        # Generate plot HTML using write_html
        fig.write_html(tmp_path, include_plotlyjs=include_plotlyjs, div_id=div_id)
        with open(tmp_path, 'r', encoding='utf-8') as f:
            plot_html = f.read()
        
        # Extract the plotly div and scripts from the generated HTML
        body_match = re.search(r'<body[^>]*>(.*?)</body>', plot_html, re.DOTALL)
        
        if body_match:
            body_content = body_match.group(1)
            
            # Find the div with our id
            div_pattern = r'<div[^>]*id="' + re.escape(div_id) + r'"[^>]*>.*?</div>'
            div_match = re.search(div_pattern, body_content, re.DOTALL)
            
            # Find all script tags in body
            script_pattern = r'<script[^>]*>.*?</script>'
            script_matches = re.findall(script_pattern, body_content, re.DOTALL)
            
            if div_match:
                plot_div = div_match.group(0)
            else:
                plot_div = f'<div id="{div_id}" class="plotly-graph-div"></div>'
            
            # Join all scripts with proper indentation
            if script_matches:
                plot_scripts = '\n\t\t\t\t\t\t\t\t\t\t'.join(script_matches)
            else:
                plot_scripts = ''
            ...
            plot_div = f'<div id="{div_id}" class="plotly-graph-div"></div>'
            plot_scripts = '<script charset="utf-8" src="https://cdn.plot.ly/plotly-2.26.0.min.js"></script>'
        
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
    
    # Build explanation section if provided
    if explanation:
        explanation_section = f"""
				<div style="margin: 3rem 0 2rem 0; padding: 2rem; background-color: #f8f9fa; border-left: 4px solid #003262; line-height: 1.6;">
					<h2 style="color: #003262; margin-top: 0;">Understanding This Visualization</h2>
					{explanation}
				</div>"""
    else:
        explanation_section = ""
    
    # Build the complete HTML with website template
...
<body class="is-preload">
	<div id="wrapper">
		<header id="header">
			<div class="inner">
				<a href="../../cee231-mechanics.html" class="logo">
					<span class="symbol"><img src="../../images/logo.png" alt="" /></span><span class="title">Facundo L. Pfeffer</span>
				</a>
		...
		<footer id="footer">
			<div class="inner">
				<section>
					<h2>Get in touch</h2>
					<ul class="icons">
						<li><a href="https://github.com/Facundo-Pfeffer" class="icon brands style2 fa-github"><span class="label">GitHub</span></a></li>
						<li><a href="https://www.linkedin.com/in/facundo-pfeffer/" class="icon brands style2 fa-linkedin"><span class="label">LinkedIn</span></a></li>
						<li><a href="mailto:facundo.pfeffer@berkeley.edu" class="icon solid style2 fa-envelope"><span class="label">Email</span></a></li>
					</ul>
				</section>
				<ul class="copyright">
					<li>&copy; Facundo L. Pfeffer. All rights reserved</li>
				</ul>
			</div>
		</footer>
	</div>
